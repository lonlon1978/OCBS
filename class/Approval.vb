...
Friend Class ApprovalDB
    Private tbl As String = "loans"

    Friend Function GetListForApproval() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            xSQL.AppendLine("     A.id, ")
            xSQL.AppendLine("     A.account_number as 'Account Number', ")
            xSQL.AppendLine("     B.display_name as 'Customer Name', ")
            xSQL.AppendLine("     A.opening_date as 'Date Opened', ")
            xSQL.AppendLine("     A.maturity_date as 'Maturity Date', ")
            xSQL.AppendLine("     A.granted_amount as 'Granted Amount' ")
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status='Opened' and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "' ")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetListForApproval(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            ...
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status='Opened' and branch_id='" + frmLogin.txtBranchID.Text.Trim() + "' ")
            xSQL.AppendLine("and CAST(" + SearchField + " as varchar(50)) like '%' + @SearchSTR + '%' ")
    ...
    Friend Function GetListOfChecks(ByVal strAccountNumber As String) As DataTable
        Dim dt As New DataTable
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            xSQL.AppendLine("     check_number as 'Check Number', ")
            xSQL.AppendLine("     bank_name as 'Bank Name', ")
            xSQL.AppendLine("     check_amount as 'Check Amount', ")
            xSQL.AppendLine("     clearing_date as 'Clearing Date' ")
            xSQL.AppendLine("FROM check_details_temp  ")
            xSQL.AppendLine("WHERE account_number = @account ")
            Try
                ...
                Dim cmd As New SqlCommand(xSQL.ToString, conn)
                cmd.Parameters.AddWithValue("@account", strAccountNumber)
                Dim da As New SqlDataAdapter(cmd)
                Dim ds As New DataSet
                da.Fill(ds)
                If ds.Tables(0).Rows.Count <> 0 Then
    ...
    Friend Function GetListToCancel() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            ...
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status in ('Opened','Approved') and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "'")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetListToCancel(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
    ...
    Friend Function GetListToDisburse() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            ...
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status in ('Approved') and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "'")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetListToDisburse(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
    ...
    Friend Function GetListToInquire() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            ...
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status in ('Current','Past Due','Litigation') and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "'")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetListToInquire(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
    ...
    Friend Function GetListToClose() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            ...
            xSQL.AppendLine("FROM  LOANS A ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status in ('Current','Past Due','Litigation') and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "' and outstanding_balance=0.00")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetListToClose(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
    ...
    Friend Function GetSAListToInquire() As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
            xSQL.AppendLine("     A.id as ID, ")
            xSQL.AppendLine("     A.account_number as 'Account Number', ")
            xSQL.AppendLine("     B.display_name as 'Customer Name', ")
            xSQL.AppendLine("     (B.numAndStreet + ' ' + B.Barangay + ' ' + b.Municipality) as Address, ")
            xSQL.AppendLine("     A.account_status as Status ")
            xSQL.AppendLine("FROM savings A  ")
            xSQL.AppendLine("inner join customer B on B.id=A.customer_id ")
            xSQL.AppendLine("where A.account_status not in ('Closed') and A.branch_id='" + frmLogin.txtBranchID.Text.Trim() + "' ")
            Try
                Dim cl = New common()
                Dim conn = cl.connect()
        ...
        Return dt
    End Function
    Friend Function GetSAListToInquire(ByVal SearchField As String, ByVal SearchSTR As String) As DataTable
        Dim dt As DataTable = Nothing
        Try
            Dim xSQL As New StringBuilder
            xSQL.AppendLine("SELECT ")
